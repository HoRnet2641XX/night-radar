...
function isBadEventTitle(value) {
  ...
  if (/^(月曜日|火曜日|水曜日|木曜日|金曜日|土曜日|日曜日|日曜日・祝日)(・|$)/.test(title)) return true
...
function parseMarkerSections(events, source, lines) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    const titleBefore = findTitleBefore(lines, index)
    const dateLines = []
    const chunk = []
    ...
    for (const dateLine of dateLines) {
      ...
      const title = markerLine.includes('実施日') ? findTitleAfterDateLine(lines, dateLine.index) : titleBefore
...
function toSeedSql(events) {
  const stores = storeDefaults
    .map((store) => `(${sql(store.id)}, ${sql(store.name)}, '都内', true, true, '13:00', '19:00', '公式イベント観測', 60)`)
    .join(',\n')
  const rows = events
    .map(
      (event) =>
  ...
  return [
    ...
    '',
    'insert into public.stores (id, name, area, has_daytime, has_night, opening_hour_day, opening_hour_night, pr_structure, trust_seed)',
    `values\n${stores}`,
